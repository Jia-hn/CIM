...
class BaseModelMerger(ABC):
    ...
    def get_transformers_auto_model_class(self):
        ...
        elif "ForCausalLM" in self.model_config.architectures[0] or self.model_config.architectures[0] in ["InternVLChatModel", "InternVLChatForCausalLM"]:
            return AutoModelForCausalLM
        elif "ForConditionalGeneration" in self.model_config.architectures[0] or self.model_config.architectures[0] in ["InternVLChatForConditionalGeneration"]:
    ...
    def save_hf_model_and_tokenizer(self, state_dict: dict[str, torch.Tensor]):
        ...
        # --- 1) 避免 save_pretrained 期间导入 deepspeed（保持你之前的绕过） ---
        orig_unwrap = MU.unwrap_model
        def _noop_unwrap(m, **kw):
            return m
        MU.unwrap_model = _noop_unwrap

        # --- 2) 修正 generation 参数检查（类级别覆盖 + 清理实例上可能残留的方法） ---
        if hasattr(model, "config"):
            # 如果之前误把方法绑在实例上，先删掉，避免被序列化进 config.json
            if "_get_non_default_generation_parameters" in getattr(model.config, "__dict__", {}):
                delattr(model.config, "_get_non_default_generation_parameters")
            # 正确做法：覆盖到“类”上（不会进入实例 __dict__）
            setattr(model.config.__class__, "_get_non_default_generation_parameters", lambda self: {})

        try:
            print(f"Saving model to {self.config.target_dir}")
            model.save_pretrained(self.config.target_dir, state_dict=state_dict)
        finally:
            MU.unwrap_model = orig_unwrap
