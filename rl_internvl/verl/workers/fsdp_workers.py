...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
        ...
        if self._is_actor or self._is_rollout:
            ...
            if os.path.isdir(local_path):
                ...
                try:
                    ...
                    cached = try_to_load_from_cache(local_path, "config.json")
                    if cached and isinstance(cached, str):
                        config_dir = os.path.dirname(cached)
                except Exception:
                    pass
            if config_dir and os.path.isdir(config_dir):
                weight_exts = {'.safetensors', '.bin', '.pt', '.msgpack', '.h5', '.onnx'}
                for fname in os.listdir(config_dir):
                    fpath = os.path.join(config_dir, fname)
                    if os.path.isfile(fpath) and os.path.splitext(fname)[1] not in weight_exts and not fname.endswith('.safetensors.index.json'):
                        with open(fpath, 'rb') as f:
                            model_config_files[fname] = f.read()
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        timing_generate.update(getattr(self.rollout_sharding_manager, "timing", {}))
