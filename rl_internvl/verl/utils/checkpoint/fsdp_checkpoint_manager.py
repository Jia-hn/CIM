...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    def save_checkpoint(self, local_path: str, hdfs_path: str = None, global_step: int = 0, max_ckpt_to_keep=None):
        ...
        if self.rank == 0:
            ...
            if self.model_config_files:
                for fname, content in self.model_config_files.items():
                    with open(os.path.join(hf_config_tokenizer_path, fname), 'wb') as f:
                        f.write(content)
        ...
        if self.should_save_hf_model:
            ...
            if self.rank == 0:
                ...
                elif "ForCausalLM" in model_config.architectures[0] or model_config.architectures[0] in ["InternVLChatModel", "InternVLChatForCausalLM"]:
                    from transformers import AutoModelForCausalLM

                    auto_model_cls = AutoModelForCausalLM
                elif "ForConditionalGeneration" in model_config.architectures[0] or model_config.architectures[0] in ["InternVLChatForConditionalGeneration"]:
                ...
                if self.model_config_files:
                    for fname, content in self.model_config_files.items():
                        with open(os.path.join(hf_local_path, fname), 'wb') as f:
                            f.write(content)
                log_with_rank(
